...
def _init_engines() -> None:
    global context_engine, chat_engine, kb, llm
    try:
        ...
        llm = OpenAILLM()
        kb = KnowledgeBase(index_name=config["index_name"])
        tokenizer = Tokenizer(config=config["tokenizer"])

        context_engine = ContextEngine(knowledge_base=kb, tokenizer=tokenizer)
        ...
        raise ConfigError("Failed to initialize engines", e)


def _init_routes(app: FastAPI) -> None:
    app.include_router(openai_api_router, prefix=f"/{API_VERSION}")
    app.include_router(context_api_router, prefix=f"/{API_VERSION}")
    app.include_router(application_router, prefix=f"/{API_VERSION}")
